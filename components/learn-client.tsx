...
export function LearnClient() {
  ...
  return (
    <div className="space-y-4">
      {/* 顶部栏 */}
      <div className="flex items-center justify-between gap-4 rounded-xl border border-[#EBEBEB] bg-white px-5 py-3 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
        <Link
          href="/"
        ...
        <h1 className="text-sm font-semibold text-[#111111]">学习页</h1>
        ...
        <div className="grid gap-8 lg:grid-cols-[0.45fr_0.55fr] lg:items-start">
          ...
          <div>
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-[#111111]">{item.title}</h2>
            </div>
            ...
            <div>
              ...
              <div className="mt-2.5 space-y-2 text-sm leading-7 text-[#555555]">
                {item.feature.length > 0 ? (
                  item.feature.map((point, pointIndex) => (
                    <p key={`${item.id}-feature-${pointIndex}`}>{point}</p>
                  ))
                ) : (
                  <p className="text-[#CCCCCC]">当前资料未录入特征原文。</p>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-2.5 space-y-2 text-sm leading-7 text-[#555555]">
                {item.note.length > 0 ? (
                  ...
                  <p className="text-[#CCCCCC]">当前资料没有备注原文。</p>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="flex flex-wrap gap-2.5">
        <button
          onClick={goPrev}
          className="rounded-lg border border-[#DADADA] bg-white px-4 py-2.5 text-sm font-medium text-[#555555] transition-colors hover:bg-[#F7F7F7]"
        >
          上一条
        </button>
        <button
